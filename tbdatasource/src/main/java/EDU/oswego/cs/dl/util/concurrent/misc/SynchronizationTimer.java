 ...
 * <li>For more explanation about tested classes, see <a href=
 * "http://gee.cs.oswego.edu/dl/classes/EDU/oswego/cs/dl/util/concurrent/intro.html">
 * Documentation for util.concurrent</a>
 * </ol>
 *
 * <p>
...
public class SynchronizationTimer {
	...
	static class TestedClass {
		...
		static final TestedClass[] classes = {
				...
				new TestedClass("LinkedQueue", ChanRNG.class, true, true, LinkedQueue.class),

				new TestedClass("WaitFreeQueue", ChanRNG.class, true, true, WaitFreeQueue.class),

				new TestedClass("BoundedLinkedQueue", ChanRNG.class, true, true, BoundedLinkedQueue.class),
				new TestedClass("BoundedBuffer", ChanRNG.class, true, true, BoundedBuffer.class),
				new TestedClass("CondVarBoundedBuffer", ChanRNG.class, true, true, CVBuffer.class),
				new TestedClass("BoundedPriorityQueue", ChanRNG.class, true, true, BoundedPriorityQueue.class),
				new TestedClass("Slot", ChanRNG.class, true, true, Slot.class),
				// new TestedClass("FIFOSlot", ChanRNG.class, true, true,
				// FIFOSlot.class),
				new TestedClass("SynchronousChannel", ChanRNG.class, true, false, SynchronousChannel.class),
				...
				// new TestedClass("Pipe", ChanRNG.class, true, true,
				// PipedChannel.class),
		};
	}
	...
	JPanel mainPanel() {
		...
		continueButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				if (!running_.get()) {
					try {
						startTestSeries(new TestSeries(nextClassIdx_.get(), nextThreadIdx_.get()));
					} catch (InterruptedException ex) {
						endTestSeries();
					}
				}
	...
	JComboBox contentionBox() {
		final Fraction[] contentionChoices = { new Fraction(0, 1), new Fraction(1, 16), new Fraction(1, 8),
				new Fraction(1, 4), new Fraction(1, 2), new Fraction(1, 1) };
	...
	JComboBox itersBox() {
		final int[] loopsPerTestChoices = { 1, 16, 256, 1024, 2 * 1024, 4 * 1024, 8 * 1024, 16 * 1024, 32 * 1024,
		...
		for (int j = 0; j < loopsPerTestChoices.length; ++j) {
			String lab = p2ToString(loopsPerTestChoices[j]) + " calls per thread per test";
	...
	JComboBox cloopBox() {
		final int[] computationsPerCallChoices = { 1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2 * 1024, 4 * 1024,
				8 * 1024, 16 * 1024, 32 * 1024, 64 * 1024, };

		JComboBox cloopComboBox = new JComboBox();

		for (int j = 0; j < computationsPerCallChoices.length; ++j) {
			String lab = p2ToString(computationsPerCallChoices[j]) + " computations per call";
	...
	JComboBox barrierBox() {
		final int[] itersPerBarrierChoices = { 1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2 * 1024, 4 * 1024,
		...
		for (int j = 0; j < itersPerBarrierChoices.length; ++j) {
			String lab = p2ToString(itersPerBarrierChoices[j]) + " iterations per barrier";
	...
	JComboBox exchangeBox() {
		final int[] exchangerChoices = { 1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, };

		JComboBox exchComboBox = new JComboBox();

		for (int j = 0; j < exchangerChoices.length; ++j) {
			String lab = p2ToString(exchangerChoices[j]) + " max threads per barrier";
	...
	JComboBox capacityBox() {
		...
		final int[] bufferCapacityChoices = { 1, 4, 64, 256, 1024, 4096, 16 * 1024, 64 * 1024, 256 * 1024,
				1024 * 1024, };

		JComboBox bcapComboBox = new JComboBox();

		for (int j = 0; j < bufferCapacityChoices.length; ++j) {
			String lab = p2ToString(bufferCapacityChoices[j]) + " element bounded buffers";
	...
	JComboBox timeoutBox() {
		...
		final long[] timeoutChoices = { 0, 1, 10, 100, 1000, 10000, 100000, };
	...
	/*
	 * void old_endTestSeries() { running_.set(false);
	...
	class TestSeries implements Runnable {
		...
		public void run() {
			...
			try {
				...
				if (t < nthreadsChoices.length && c < TestedClass.classes.length) {

					for (;;) {

						// these checks are duplicated in OneTest, but added
						// here
				...
				synchronized (RNG.constructionLock) {
					...
					if (chanCls == null) {
						...
						if (nthreads == 1) {
							...
							PCTestLoop l = new PCTestLoop(single.getDelegate(), single, pshr, iters, barrier, shared,
									shared);
					...
					if (echoToSystemOut.get()) {
						System.out.print(entry.name + " " + nthreads + "T " + pshr + "S " + RNG.computeLoops.get()
								+ "I " + RNG.syncMode.get() + "Lm " + RNG.timeout.get() + "TO " + RNG.producerMode.get()
								+ "Pm " + RNG.consumerMode.get() + "Cm " + RNG.bias.get() + "B "
								+ DefaultChannelCapacity.get() + "C " + RNG.exchangeParties.get() + "Xp "
...
class TestLoop {
	...
	public TestLoop(RNG sh, RNG pri, Fraction pshr, int it, CyclicBarrier br) {
		...
		if (num == 0 || primary == shared) {
			...
			// denom slots is too few. iters is too many. an arbitrary
			// compromise is:
...
class AClongRNG extends RNG {
	protected final SynchronizedLong acurrent_ = new SynchronizedLong(nextSeed());
