...
public abstract class SwingWorker extends ThreadFactoryUser implements Runnable {
	 ...
	 * 
	 * @param msecs
	 *            timeout in milliseconds, or <code>0</code> for no time limit.
	 */
	...
	public synchronized void interrupt() {
		if (thread != null) {
			/*
			 * Try-catch is workaround for JDK1.2 applet security bug. On some
			 * platforms, a security exception is thrown if an applet interrupts
			 * a thread that is no longer alive.
			 */
			try {
				thread.interrupt();
			} catch (Exception ex) {
	 ...
	 *                if the constructing thread encountered an exception or was
	 *                interrupted.
	 */
